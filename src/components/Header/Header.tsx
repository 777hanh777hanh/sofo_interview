...
const Header = () => {
	...
	useEffect(() => {
		if (isCartHovered) {
			...
			return () =>
				window.removeEventListener('resize', calculateCartPosition);
		}
	}, [isCartHovered]);
	...
	return (
		<>
				...
				<div className='flex items-center justify-between gap-8 lg:gap-12'>
					...
					<div className='flex items-center gap-4 lg:gap-8'>
						...
						<div className='flex items-center gap-2'>
							...
							/>
							<p
								className='text-sm !leading-[1] font-medium text-[#1C252E] uppercase select-none
									lg:text-base'
							>
						...
						>
							<div className='relative flex items-center gap-2'>
								...
								<p
									className='absolute -top-2.5 -right-2.5 flex h-6 w-6 items-center justify-center
										rounded-full bg-[#FF5630] text-[12px] font-medium text-white'
								>
			...
			{isCartHovered && itemHot.length > 0 && (
				...
				>
					<Card
						className='w-auto max-w-[220px] overflow-hidden rounded-lg bg-white
							shadow-[0_8px_16px_0_rgba(3,117,243,0.16)]'
						{...itemHot[0]}
					/>
				</div>
			)}
